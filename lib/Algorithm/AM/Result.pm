...
#pod =head2 C<analogical_set>
#pod
#pod The analogical set is the set of items from the training set that
#pod had some effect on the item classification. The analogical effect of
#pod an item in the analogical set is the score it contributed towards
#pod a classification matching its own class label.
#pod
#pod This method returns the items in the analogical set along with their
#pod analogical effects, in the following structure:
#pod
#pod  { 'item_id' => {'item' => item, 'score' => score}
#pod
#pod C<item> above is the actual item object. The item_id is used so that
#pod the analogical effect of a particular item can be found quickly:
#pod
#pod  my $set = $result->analogical_set;
#pod  print 'the item's analogical effect was '
#pod      . $set->{$item->id}->score;
#pod
#pod =cut
sub analogical_set {
    my ($self) = @_;
...
#pod =head2 C<gang_effects>
#pod
#pod Return a hash describing gang effects. Gang effects are similar to
#pod analogical sets, but the total effects of entire subcontexts and
#pod supracontexts are also calculated and printed.
#pod
#pod TODO: details, details! Maybe make a gang class to hold this structure.
#pod
#pod =cut
sub gang_effects {
    my ($self) = @_;
